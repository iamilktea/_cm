...
* **作者**：[Your Name / GitHub ID]
* **課程**：運算數學
* **學期**：2025 Fall (或依照實際學期填寫)
* **完成狀況**：已全數完成並繳交。

## ⚠️ 來源與完成方法宣告 (Declaration)

根據期末作業要求，在此說明本專案程式碼的完成方式：
* **核心邏輯**：大部分程式碼架構由本人構思，並使用 AI 工具 (ChatGPT/Gemini) 協助生成基礎函式庫或數學公式的轉換。
* **驗證與修改**：所有由 AI 生成的程式碼皆經過本人閱讀、理解、除錯 (Debug) 並撰寫測試案例 (Test Cases) 驗證，確保符合題目數學要求。
* **文件撰寫**：README 與數學原理說明由本人整理，部分數學推導參考維基百科與 AI 的解釋。
